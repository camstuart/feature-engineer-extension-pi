...
function buildVaguePrompt(inputs: ReqGatheringPromptInputs): string {
  ...
  const lines: string[] = [
    ...
    "",
    "After gathering the answers, synthesise them into a short summary (success criteria, constraints, and out-of-scope items) and use ONE `ui.confirm` to verify the whole summary before moving on — do not confirm each criterion individually.",
    "",
    "**STEP 4 — User Story Drafting**",
    "",
    "For each actor and journey from STEP 2, draft 1-3 user stories using the As/I want/so that format. For each story, draft 2-4 acceptance criteria. After drafting all stories for this round, present them together in one summary and use ONE `ui.confirm` to verify the full set captures the intent — do not confirm each story individually. Adjust wording based on feedback before moving on.",
    "",
    "**STEP 5 — Goals & Requirements (with traceability)**",
    "",
    "Derive 3-6 goals from the user stories. Each goal should map to at least one user story. After deriving all goals, present them together and use ONE `ui.confirm` to verify the full set — do not confirm each goal individually.",
    "",
